 ...
 *
 */
public class NewMOFA extends Algorithm {
	/*
	 * 萤火虫群体的个数
	 */
	...
	private List<SolutionSet> eArchives_;
	/*
	 * 萤火虫历史最优解的Solution数组
	 */
	 ...
	 * @param problem
	 */
	public NewMOFA(Problem problem) {
		super(problem);
...
//		alpha_                       = ((Double)getInputParameter("alpha")).doubleValue();
		...
		eArchives_                   = new ArrayList<SolutionSet>();
...
//	  double  C2 = PseudoRandom.randDouble(1.5,2.0);
	  double  W  = PseudoRandom.randDouble(); 
		...
		for(int var=0; var<fireflyI.length;var++){
			 move_[i][var] =   move_[i][var] +
	    				W*beta_*Math.exp(-gammar_*distance)*
	                   (fireflyI[var].getValue() - 
	                              fireflyJ[var].getValue())
	                              +(1-W)*beta_*Math.exp(-gammar_*distance1)*
